...
from funciones import validMatriz, mostrarMatriz, llenarCeldaOcupadas, cantCeldasVacias
...
while True:
    ...
    matriz[0][rand]=1# coloco una reina en una pocicon aleatoria de la primer fila menos en las esquinas
    ...
    for i in range(1,8):
        ...
        for j in range(0,8): #recorro toda la fila
            if (matriz[i][j]==0):# veo cuales son los lugares vacios
                tempMatriz[i][j]=1
                if (cantMin <= cantCeldasVacias(llenarCeldaOcupadas(tempMatriz))):#cuento la cantidad
                    if (cantMin < cantCeldasVacias(llenarCeldaOcupadas(tempMatriz))):# guardo en la lista las posiciones que con la que se obtine la mayor cantidad de celdas vacias
                        valorJ=[]
                        valorJ.append(j)
                    else:
                        valorJ.append(j)
                    cantMin=cantCeldasVacias(llenarCeldaOcupadas(tempMatriz))# guardo la cantidad cantidad de posiciones vacias
                    nvacias=0
            tempMatriz = copy.deepcopy(matriz)

        if (nvacias==0):# veo si hay lugares vacios
            alat=valorJ[int(uniform(0,valorJ.__len__()))]
            matriz[i][alat]=1 #coloco la reina en un lugar aleatorio
...
for i in range(0,8):#quito todos los 2 de la matriz
        for j in range(0,8):
            if matriz[i][j]==2:
                matriz[i][j]=0        
mostrarMatriz(matriz)